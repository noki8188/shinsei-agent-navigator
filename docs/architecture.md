...
`v0.34.0` では、`v0.33.0` の LLM hardening を土台に、representative eval の網羅性と trace 表示の一貫性を高めました。
...
flowchart LR
    ...
    RA --> R1
    TB --> R1
    CC -. env switch .-> L1["OpenAI Responses API implementation (v0.34.0 baseline)"]
    CA -. env switch .-> L1
    DA -. env switch .-> L1
    L1 -. fallback .-> R1
```
...
- `v0.34` では classification / clarification / rule references / review に加えて、実行 backend と fallback 状況を trace から追いやすくした
